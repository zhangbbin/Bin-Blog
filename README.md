# Bin-Blog

[![.NET Status](https://img.shields.io/badge/.NET-10.0%20(Preview)-purple?style=flat-square&logo=dotnet)](https://dotnet.microsoft.com/)
[![Blazor](https://img.shields.io/badge/Blazor-Server-512bd4?style=flat-square&logo=blazor)](https://dotnet.microsoft.com/apps/aspnet/web-apps/blazor)
[![Style](https://img.shields.io/badge/Style-Minimalist-black?style=flat-square)](https://github.com/zhangbbin/bin-blog)

> *Minimalist. Functional. Pure C#.*

一个基于 **.NET 10** 与 **Blazor Server** 构建的个人博客。
没有复杂的 CMS，没有追踪器，只有文字与代码。

### ✨ Core
* **Design**: 极简主义 UI，黑白灰的秩序感。
* **Theme**: 昼夜模式自动切换，致敬系统偏好。
* **Tech**: Entity Framework Core + Markdig + Giscus。
* **Pure**: 拒绝臃肿的前端框架，C# 贯穿全栈。

### 🚀 Start
只需两步，唤醒它：

```bash
git clone https://github.com/zhangbbin/bin-blog.git
cd bin-blog
dotnet watch
```

### 📚 Documentation
* **[快速开始指南 / Quick Start Guide](QUICK-START-GUIDE.md)** - 5 分钟上手开发
* **[AI 开发提示词 / AI Development Prompts](AI-DEVELOPMENT-PROMPTS.md)** - 使用 AI 助手快速实现功能

### 🎯 Roadmap
- [x] 博客文章列表与详情页
- [x] Markdown 渲染与代码高亮
- [x] 标签筛选功能
- [x] 昼夜主题切换
- [x] Giscus 评论系统
- [ ] 用户认证与授权
- [ ] 后台管理面板
- [ ] 全站搜索
- [ ] SEO 优化与 RSS
- [ ] 图片上传管理

详细开发计划请查看 [AI-DEVELOPMENT-PROMPTS.md](AI-DEVELOPMENT-PROMPTS.md)
